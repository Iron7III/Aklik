...
exports.run = async (client, message, args, FortniteAPIComClient, FortniteAPIIoClient, {assets}) => {
    ...
    let ServerInfo_Moderation = [
        ...
        `> **AFK Timeout ➜ **\`${(date.getHours()-1)>9?date.getHours()-1:`0${date.getHours()}`}:${date.getMinutes()>9?date.getMinutes():`0${date.getMinutes()}`}:${date.getSeconds()>9?date.getSeconds():`0${date.getSeconds()}`}\``,
