...
exports.run = async (client, message, args, Fortnite) => {
  module.exports = {
    ...
    async generateShop(shop, watermark) {
      ...
      console.log("[Fort-Shop] Made by Im2rnado. For support, join discord.gg/carbide");
      ...
      console.log(`[CANVAS] Width ${canvasWidth} x Height ${canvasHeight}`);
      ...
      for (const i of keys) {
        ...
        for (const item of items) {
          ...
          console.log(`[ITEMS] Drawing ${item.name}`);
          ...
          if (item.images.background) {
            try {
              ...
              console.log(`Could not load image for ${item.name}`);
          ...
          if (item.effects && item.effects.length) {
            try {
              ...
              console.log(`Could not load Gameplay Tag ${item.effects[0].split(".").pop()}`);
            }
          }
      ...
      const attach = new Discord.MessageAttachment(buf, `shop-cataba.jpg`)
