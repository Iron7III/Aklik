                    ...
                    if(message.member.roles.highest.comparePositionTo(member.roles.highest) <= 0){
                        ...
                        if(!member.bannable){
                            ...
                            message.guild.members.ban(member.id, {reason: reason})
                                .then(()=>{
                                    embed.setDescription(`_${member.user.username}#${member.user.discriminator} ha sido baneado con motivo: \`${args[1]?args.slice(1).join(/ +/g):'No hay motivo.'}\`_`).setColor('#57F287')
                                    message.channel.send({embed: embed})
                                })
                                .catch(err => {
